...
When you install a Medusa application, you get a fully-fledged commerce platform with the Framework for customization. The Medusa application's commerce features are built around [Commerce Modules](https://docs.medusajs.com/commerce-modules), which are available out of the box.
...
Refer to the [Data Models](https://docs.medusajs.com/learn/fundamentals/modules#1-create-data-model) documentation to learn more.
...
Find all methods generated by the `MedusaService` in [the Service Factory](https://docs.medusajs.com/service-factory-reference) reference.
...
1. A [workflow](https://docs.medusajs.com/learn/fundamentals/workflows) with steps that define the feature's business logic.
2. An [API route](https://docs.medusajs.com/learn/fundamentals/api-routes) that exposes the workflow's functionality to client applications.
...
2. An async function that receives two parameters:
   ...
   - An object with properties including the [Medusa container](https://docs.medusajs.com/learn/fundamentals/medusa-container), which is a registry of Framework and commerce tools accessible in the step.
...
1. Check if any of the images to create is a thumbnail using [when-then](https://docs.medusajs.com/learn/fundamentals/workflows/conditions).
...
To validate the body parameters of requests sent to the API route, apply a [middleware](https://docs.medusajs.com/learn/fundamentals/api-routes/middlewares).
...
In the API route, you resolve [Query](https://docs.medusajs.com/learn/fundamentals/module-links/query), which retrieves data across modules.
...
To send requests to the Medusa server, you'll use the [JS SDK](https://docs.medusajs.com/js-sdk). It's already installed in your Medusa project, but you need to initialize it before using it in your customizations.
...
Learn more about the initialization options in the [JS SDK](https://docs.medusajs.com/js-sdk) reference.
...
1. `uploadFilesMutation`: A mutation that uploads files using Medusa's existing API route for uploads. This will upload the images to the [configured File Module Provider](https://docs.medusajs.com/infrastructure-modules/file#what-is-a-file-module-provider).
...
You render a modal using the `FocusModal` component from [Medusa UI](https://docs.medusajs.com/ui). The modal displays the `CategoryImageGallery` component on the left and the `CategoryImageUpload` component on the right.
...
Images are uploaded to the [configured File Module Provider](https://docs.medusajs.com/infrastructure-modules/file#what-is-a-file-module-provider). If you haven't configured one, images will be uploaded to the `static` folder in your Medusa project.
...
- [useQueryGraphStep](https://docs.medusajs.com/references/helper-steps/useQueryGraphStep): Retrieve the file IDs of the images
- [deleteFilesWorkflow](https://docs.medusajs.com/references/medusa-workflows/deleteFilesWorkflow): Delete the files from storage.
...
2. Prepare the file IDs using [transform](https://docs.medusajs.com/learn/fundamentals/workflows/variable-manipulation).
...
In the last step, you'll update the [Next.js Starter Storefront](https://docs.medusajs.com/nextjs-starter) to:
...
You can do this by creating a [read-only link](https://docs.medusajs.com/learn/fundamentals/module-links/read-only). A read-only link allows you to retrieve data related to a model from another module without compromising [module isolation](https://docs.medusajs.com/learn/fundamentals/modules/isolation).
...
To learn more about the commerce features Medusa provides, check out [Commerce Modules](https://docs.medusajs.com/commerce-modules).
